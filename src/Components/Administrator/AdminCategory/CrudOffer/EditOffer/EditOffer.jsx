...
import { editProductsOffers, getProductsOffers, getProductsOffersById } from '../../../../../service/indexProductsOffers';
...
const EditOffer = () => {
  ...
  const getOneOffer =  async(_id) => {
    ...
    setOfferById(data.getDataOneOffersById)
    reset()
  }

  const editOneOffer =async (datas) => {
    const id = (offerById.map(offer => offer._id))
    editProductsOffers(id, datas)
    reset()
    setSuccessMessage('Oferta editada exitosamente');
  }
          ...
          <TableBody>
            {offerById.map((row) =>(
                <StyledTableRow key={row._id}>
                  ...
                  <StyledTableCell >
                        <Button variant="contained" type='submit'onClick={handleSubmit(editOneOffer)}>Modificar</Button>
                  </StyledTableCell>
                </StyledTableRow>
              ))}
          </TableBody>
          ...
          <TableBody>
            {allOffer.map((row) => (
              <StyledTableRow key={row._id}>
                ...
                <StyledTableCell ><Button variant="contained" color="primary" onClick={()=>getOneOffer(row._id)}>Editar</Button></StyledTableCell>
              </StyledTableRow>
            ))}
          </TableBody>
        </Table>
